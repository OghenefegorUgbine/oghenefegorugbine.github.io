...
| --- | --- | --- | --- |
| `degree-certificates/` | **Yes** | Public copies of the Erasmus Mundus (META 4.0) joint master's certificate, the École Centrale de Lyon master's diploma, the École des Mines de Saint-Étienne master's diploma, and the CRUTECH bachelor's degree certificate | Published in full at your request, exactly as issued, with nothing on the certificates covered or removed. These are the degree certificates and diplomas only; no transcripts are included. Note: some of these documents show your date of birth and birthplace, since that's printed on the certificate itself. The earlier Centrale Lyon "certificate of achievement" file is still in this folder but is no longer linked from the site, replaced by the actual master diploma. |
